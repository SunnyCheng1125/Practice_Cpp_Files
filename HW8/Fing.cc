...
Fing *Fing::create(const string &fingPathC) {
	struct stat statbuf;
    const char *fingp = fingPathC.c_str();
    int result = lstat(fingp, &statbuf);

    if (result != 0) {
        throw string(" lstat file path: " + fingPathC + " failed");
    }
    switch (statbuf.st_mode & S_IFMT) {
        case S_IFREG:
            return new Regular(fingPathC);
        case S_IFDIR:
            return new Directory(fingPathC);
        case S_IFLNK:
			return new Symlink(fingPathC);
        default:
            throw string("Wrong type : "+fingPathC);
    }
}
